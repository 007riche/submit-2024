...
//        final Integer DEFAULTPORT = 1099;
        ...
        try {
            ...
            URL fileUrl = Server.class.getClassLoader().getResource("security.policy"); //  security.policy
            assert fileUrl != null;
            String path = Paths.get(fileUrl.toURI()).toAbsolutePath().toString();
            String filePath = new File(fileUrl.getFile()).getAbsolutePath();

//            String policyfile = Server.class.getClassLoader().getResourceAsStream("security.policy").toString();
...
//            System.err.println("Server UP ...");
            ...
            System.err.println(pub.getName());
            System.err.println(pubSpecy.getSpecyName());
            pubCabinet.getAllPatients().stream().forEach( animal -> {
