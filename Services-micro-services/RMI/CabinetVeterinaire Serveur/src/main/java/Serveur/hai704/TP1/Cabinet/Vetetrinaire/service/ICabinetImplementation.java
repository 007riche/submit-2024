...
public class ICabinetImplementation extends UnicastRemoteObject implements ICabinet {
    ...
    protected List<Specy> specyList;
...
//        this.patientsCabinet = new ArrayList<AnimalWrapperServer>();
        this.patientsCabinet = new ArrayList<>() {{
            add(new Animal("Loulou", "Barth", "chiuaua", new Specy("Dog", 300.0),new FollowUpFile()));
            add(new Animal("Rox", "Dexi", "Zua", new Specy("Cat", 500.0),new FollowUpFile()));
            add(new Animal("Booggy", "Arcana", "Polar", new Specy("Grizzly", 70000.0),new FollowUpFile()));
            add(new Animal("Rash", "Gondi", "Dandy", new Specy("Suricate", 234.0),new FollowUpFile()));
        }};

        specyList=new ArrayList<>();
        this.patientsCabinet.stream().forEach(animal -> {
                if(!this.specyList.contains(animal.getSpecy()))
                    this.specyList.add(animal.getSpecy());
            });
...
//                    throw new RuntimeException(e);
    ...
    @Override
    public List<Specy> getAllSpecies() throws RemoteException {
        return specyList;
    }
